        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          <div className="lg:col-span-2 space-y-16">
            ...
            <section className="animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex items-start gap-4 p-5 bg-emerald-50/60 rounded-lg border border-emerald-100 hover:border-emerald-300 transition-colors">
                ...
                <div className="flex items-start gap-4 p-5 bg-emerald-50/60 rounded-lg border border-emerald-100 hover:border-emerald-300 transition-colors">
                  <MapPin className="text-emerald-700 mt-1 shrink-0" size={20} />
                  <div>
                    <h4 className="font-semibold text-emerald-950 text-sm">
                      {language === 'en' ? 'Address' : 'Adresse'}
                    </h4>
                    <p className="text-gray-700 font-medium mt-1">{journalInfo.address}</p>
                  </div>
                </div>

                <div className="flex items-start gap-4 p-5 bg-emerald-50/60 rounded-lg border border-emerald-100 hover:border-emerald-300 transition-colors">
                  <Phone className="text-emerald-700 mt-1 shrink-0" size={20} />
                  <div>
                    <h4 className="font-semibold text-emerald-950 text-sm">
                      {language === 'en' ? 'Phone' : 'Téléphone'}
                    </h4>
                    <div className="mt-1 space-y-1">
                      {journalInfo.phones.map((phone, i) => (
                        <a
                          key={i}
                          href={`tel:${phone.replace(/\s+/g, '')}`}
                          className="block text-gray-700 hover:text-emerald-700 font-medium transition-colors"
                        >
                          {phone}
                        </a>
                      ))}
                ...
                <div className="flex items-start gap-4 p-5 bg-emerald-50/60 rounded-lg border border-emerald-100 hover:border-emerald-300 transition-colors">
                  <Mail className="text-emerald-700 mt-1 shrink-0" size={20} />
                  <div>
                    <h4 className="font-semibold text-emerald-950 text-sm">Email</h4>
                    <a
                      href={`mailto:${journalInfo.contactEmail}`}
                      className="block text-gray-700 hover:text-emerald-700 font-medium break-all mt-1 transition-colors"
                    >
          ...
          <div className="space-y-12">
            ...
            <div className="p-8 border border-gray-200 rounded-sm text-center bg-white shadow-sm">
              <MapPin size={32} className="mx-auto text-emerald-700 mb-4" />
              ...
              <p className="text-gray-500 text-sm mb-6 leading-relaxed">
                ...
                {journalInfo.address}<br />
                <a href={`mailto:${journalInfo.email}`} className="text-emerald-700 hover:underline">
                  {journalInfo.email}
                </a>
              </p>
